...
namespace WalkerChiu\DeviceRFID\Models\Observers;

class ReaderRegisterLangObserver
{
    /**
    ...
    {
        $query =
            config('wk-core.class.device-rfid.readerRegisterLang')
                ::withTrashed()
        ...
        if (
            config('wk-device-rfid.soft_delete')
            && (
                config('wk-core.lang_log')
                || config('wk-device-rfid.lang_log')
            )
        ) {
