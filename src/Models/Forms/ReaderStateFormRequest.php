...
class ReaderStateFormRequest extends FormRequest
{
    ...
    public function rules()
    {
        ...
        if (
            ...
            $rules = array_merge($rules, ['id' => ['required','integer','min:1','exists:'.config('wk-core.table.device-rfid.readers_states').',id']]);
        } elseif ($request->isMethod('post')) {
            $rules = array_merge($rules, ['id' => ['nullable','integer','min:1','exists:'.config('wk-core.table.device-rfid.readers_states').',id']]);
        }
